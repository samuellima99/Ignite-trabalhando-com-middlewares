...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro || user.todos.length === 10) {
    return response
      .status(403)
      .json({ error: "The user has reached the limit of all free plans!" });
  }
}
...
function checksTodoExists(request, response, next) {
  ...
  const userExists = users.find((user) => user.username === username);
  const idIsTodo = userExists?.todos.find((todo) => todo.id === id);
  ...
  if (!idIsTodo) {
    return response
      .status(404)
      .json({ error: "Id doesn't belong to any whole!" });
  }

  request.todo = idIsTodo;
  request.user = userExists;
